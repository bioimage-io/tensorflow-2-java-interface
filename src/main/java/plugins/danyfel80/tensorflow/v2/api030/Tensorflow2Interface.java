package plugins.danyfel80.tensorflow.v2.api030;
...
import org.bioimageanalysis.icy.tensorflow.v2.api030.tensor.ImgLib2Builder;
...
public class Tensorflow2Interface implements DeepLearningInterface
{
	...
	public static List<Tensor> fillOutputTensors(List<org.tensorflow.Tensor> outputNDArrays, List<Tensor> outputTensors) throws RunModelException{
		...
		for (int i = 0; i < outputNDArrays.size(); i ++) {
			outputTensors.get(i).setData(ImgLib2Builder.build((TType) outputNDArrays.get(i)));
		}
		return outputTensors;
	}
